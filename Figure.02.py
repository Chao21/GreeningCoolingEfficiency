"""
Figure 2: spatial patterns of LST-LAI sensitivity and its trend.

This script is a cleaned, reproducible version of the original Figure02 code.
It preserves the original layout:
    a. global map of sensitivity trend,
    b. inset map of mean sensitivity,
    c. climate-zone distributions of mean sensitivity and trend,
    d. climate-space hexbin of mean sensitivity,
    e. climate-space hexbin of sensitivity trend.
...
class FigureConfig:
    ...
    output_figure: Path = Path("../figure/202606/Fig.02_PixelTrend_Map.png")

    regions: tuple[str, ...] = ("Global", "Tropical", "Temperate", "Arid", "Boreal")
    climate_zone_names: tuple[str, ...] = ("Tropical", "Arid", "Temperate", "Boreal")
    colors: tuple[str, ...] = ("#000000", "#313691", "#6497c3", "#f68758", "#bc212b")
...
def sensitivity_paths(cfg: FigureConfig) -> dict[str, Path]:
    ...
    return {
        ...
        "sensitivity_trend": root / f"Sens_trend_{cfg.time_scale}_LST{cfg.lst_var}_LAI_{cfg.product}_1d.nc",
        "annual_region": root / f"{prefix}_regionAgg.csv",
        "season_region": root / f"Sensitivity_Season_LST{cfg.lst_var}_LAI_{cfg.product}_regionAgg_NSharmonized.csv",
        "dryhumid_region": root / f"{prefix}_regionDryhumid.csv",
        "climate_zone": Path("koppen_geiger_4class_1d.nc"),
        "climate_data": Path("ERA5-Land_Annual_2000_2024_1deg.nc"),
    }


def load_inputs(cfg: FigureConfig):
    ...
    trend = xr.open_dataset(paths["sensitivity_trend"])
    ...
    climate = (
        xr.open_dataset(paths["climate_data"])
        .sel(time=slice("2001", "2024"))
...
def climate_trend_sign_proportions(
    ...
    *,
    ...
    if weights is None:
        ...
        return {key: float(mask.sum().item()) / total * 100 if total > 0 else np.nan for key, mask in masks.items()}

    weights, valid = xr.align(weights, valid, join="inner")
    total = float(weights.where(valid).sum(skipna=True).item())
    return {
        key: float(weights.where(mask).sum(skipna=True).item()) / total * 100 if total > 0 else np.nan
...
def plot_regional_distribution_boxes(
    ...
    *,
    legend: bool = False,
    title: str = "",
    ylabel_mean: str = r"$\partial$LST / $\partial$LAI mean" "\n" r"[K ($\mathrm{m}^2\,\mathrm{m}^{-2}$)$^{-1}$]",
    ylabel_trend: str = r"$\partial$LST / $\partial$LAI trend" "\n" r"[K ($\mathrm{m}^2\,\mathrm{m}^{-2}$)$^{-1}$ decade$^{-1}$]",
):
    ...
    for i, name in enumerate(region_names):
        ...
        for element in ("medians", "whiskers"):
            for item in bp_mean[element] + bp_trend[element]:
                item.set(color="k" if element == "medians" else "gray", linewidth=0.7 if element == "medians" else 0.5)
    ...
    ax_trend.set_ylabel(ylabel_trend, color=colors["trend"], fontsize=12, rotation=270, labelpad=30)
...
def build_figure(cfg: FigureConfig):
    ...
    cax_inset = fig.add_axes(
        [
            ax_inset.get_position().x0 + 0.03,
            ax_inset.get_position().y0 + 0.05,
            0.01,
            0.1,
        ]
    )
