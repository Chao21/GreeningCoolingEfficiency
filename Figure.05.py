"""
Figure 5: decomposition of LAI-change-induced LST effect.

This script is a cleaned, reproducible version of the original Figure05 code.
It preserves the final six-panel layout:
    a. global dLST* decomposition time series,
    b. climate-zone trend decomposition,
    c. total dLST* trend map,
    d. dominant trend term map,
    e. LAI-driven dLST* trend map,
    f. sensitivity-driven dLST* trend map.
...
class FigureConfig:
    ...
    output_figure: Path = Path("../figure/202606/Fig.05_dLST_Decomposition.png")
    folder: Path = Path("processed/dLST_Decompose_20260223/ConsistentMask_20260603/GLASS_Annual_LSTdailymean")
...
def load_inputs(cfg: FigureConfig) -> dict[str, object]:
    ...
    df_region = pd.read_csv(folder / f"{prefix}_ClimateZone_TS.csv", parse_dates=["time"])
    df_region_trend = pd.read_csv(folder / f"{prefix}_ClimateZone_Trend.csv")

    return {
        "df_region": df_region,
        "df_region_trend": df_region_trend,
        "ds_total": xr.open_dataset(folder / f"{prefix}_Trend_1deg_dLST_total.nc"),
        "ds_lai": xr.open_dataset(folder / f"{prefix}_Trend_1deg_dLST_LAI_driven.nc"),
        "ds_sens": xr.open_dataset(folder / f"{prefix}_Trend_1deg_dLST_Sens_driven.nc"),
    }


def select_series(df: pd.DataFrame, *, variable: str | None = None, region: str = "Global", group: str = "annual") -> pd.DataFrame:
...
def build_region_slope_table(df_region: pd.DataFrame, *, regions: Sequence[str], group: str = "annual") -> pd.DataFrame:
...
def plot_global_timeseries(ax, df_region: pd.DataFrame, *, colors: Sequence[str]):
    ...
    for label, variable, color in series:
        ...
        ax.plot(time.year, y, marker="o", color=color, alpha=0.5, linestyle="--", linewidth=1.2, markersize=3.5, label=LABEL_SIGMA[label])

        slope, pval, intercept, stderr, _ = linear_trend(time, y)
        if np.isfinite(slope):
            ax.plot(time.year, intercept + slope * x, color=color, linewidth=2.2)
            annotation_lines.append((label, f"{slope * 10:+.3f} K decade$^{{-1}}$$^{{{p_to_star(pval)}}}$", color))
    ...
    ax.legend(frameon=False, ncol=3, columnspacing=1, loc="lower right", bbox_to_anchor=(0.95, -0.33))
...
def plot_climate_zone_decomposition(
    ...
    *,
    ...
    def draw_points(xpos, yval, yerr, pval, color, label, zorder):
        ax.errorbar(xpos, yval, yerr=yerr, fmt="none", ecolor=color, elinewidth=0.9, capsize=2, alpha=0.9, zorder=zorder)
        pval = np.asarray(pval)
        sig = np.isfinite(pval) & (pval < 0.05)
        nonsig = np.isfinite(pval) & (pval >= 0.05)
        missing = ~np.isfinite(pval)
        ax.plot(xpos[sig], yval[sig], "o", markerfacecolor=color, markeredgecolor=color, markersize=marker_size, linestyle="none", label=label, zorder=zorder + 1)
        ax.plot(xpos[nonsig], yval[nonsig], "o", markerfacecolor="white", markeredgecolor=color, markeredgewidth=1.0, markersize=marker_size, linestyle="none", zorder=zorder + 1)
        ax.plot(xpos[missing], yval[missing], "o", markerfacecolor="white", markeredgecolor=color, markeredgewidth=0.8, alpha=0.45, markersize=marker_size, linestyle="none", zorder=zorder + 1)
    ...
    for i in range(len(regions)):
        if np.isfinite(lai[i]) and np.isfinite(sens[i]) and abs(lai[i]) > 1e-12:
            ...
            ratio_text = f"{ratio:+.{ratio_digits}f}%"
            ratio_color = "#b0202e" if ratio > 0 else "#6aa8cc"
        else:
            ratio_text = "NA"
            ratio_color = "0.35"
        ax.text(x[i], ratio_y, ratio_text, color=ratio_color, ha="center", va="center", fontsize=10, clip_on=False, zorder=6)

    ax.set_xticks(x)
    ax.set_xticklabels(regions, fontsize=10)
    ax.set_ylabel(r"K decade$^{-1}$")
    ax.set_title("Decomposition by climate zone")
    ax.tick_params(direction="in")

    ax.add_patch(Rectangle((-0.4, 0.048), 4.8, 0.018, facecolor="none", edgecolor="gray", linestyle="--", linewidth=1.0))
    ax.annotate(
    ...
    if show_legend:
        color_handles = [
            Line2D([0], [0], marker="o", color="none", markerfacecolor=color_map["total"], markeredgecolor=color_map["total"], markersize=5, label=LABEL_DELTA["Total"]),
            Line2D([0], [0], marker="o", color="none", markerfacecolor=color_map["lai"], markeredgecolor=color_map["lai"], markersize=5, label=LABEL_DELTA["LAI-driven"]),
            Line2D([0], [0], marker="o", color="none", markerfacecolor=color_map["sens"], markeredgecolor=color_map["sens"], markersize=5, label=LABEL_DELTA["Sens-driven"]),
        ]
        sig_handles = [
            Line2D([0], [0], marker="o", color="black", markerfacecolor="black", markeredgecolor="black", linestyle="none", markersize=5, label="$p$<0.05"),
            Line2D([0], [0], marker="o", color="black", markerfacecolor="white", markeredgecolor="black", linestyle="none", markersize=5, label="$p$>=0.05"),
        ]
        leg = ax.legend(handles=color_handles, frameon=False, ncol=3, loc="lower left", bbox_to_anchor=(-0.01, -0.33))
        ax.add_artist(leg)
        ax.legend(handles=sig_handles, frameon=False, ncol=1, handlelength=0.8, handletextpad=0.45, loc="lower right", bbox_to_anchor=(1.0, 0))

    return ax


def count_sig_sign_categories(trend: xr.DataArray, pval: xr.DataArray, pthr: float = 0.05):
    """Count pixels by trend sign and significance."""
...
def add_pdf_inset(ax, trend: xr.DataArray, pval: xr.DataArray, *, cmap, norm, inset_bbox=(0.07, 0.05, 0.22, 0.22), bins=20):
    """Add a compact global distribution inset for trend values."""
    ax_in = inset_axes(ax, width="100%", height="100%", bbox_to_anchor=inset_bbox, bbox_transform=ax.transAxes, loc="lower left", borderpad=0)
    ...
    ax_in.bar(centers, density, width=widths, align="center", color=cmap(norm(centers)), edgecolor="none", alpha=0.9, zorder=2)
    ax_in.plot(centers, density, color="k", linewidth=1, zorder=3)
    ax_in.axhline(0, color="k", linewidth=1.0, zorder=4)
    ax_in.text(0.02, 0.88, f"{np.mean(values < 0) * 100:.0f}%", color="tab:blue", transform=ax_in.transAxes, ha="left", va="top", fontsize=11)
    ax_in.text(0.98, 0.88, f"{np.mean(values > 0) * 100:.0f}%", color="tab:red", transform=ax_in.transAxes, ha="right", va="top", fontsize=11)
...
def plot_trend_map(ax, ds: xr.Dataset, *, title: str, cmap, norm, step: int = 3, add_inset: bool = False):
    ...
    ax.scatter(xx[mask], yy[mask], s=0.3, color="k", marker=".", alpha=0.7, transform=ccrs.PlateCarree(), zorder=10)
...
def plot_dominance_map(ax, ds_lai: xr.Dataset, ds_sens: xr.Dataset, *, eps: float = 0.0):
    ...
    im = ax.pcolormesh(cat["lon"], cat["lat"], cat, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, shading="auto")
    ax.coastlines(linewidth=0.5, color="0.3")
    ax.add_feature(cfeature.BORDERS.with_scale("110m"), linewidth=0.3, edgecolor="0.4")
    ax.set_title("Dominance of trend terms")
    ax.set_extent([-180, 180, -60, 90], crs=ccrs.PlateCarree())

    ax_in = inset_axes(ax, width="100%", height="100%", bbox_to_anchor=(0, 0.04, 0.39, 0.39), bbox_transform=ax.transAxes, loc="lower left", borderpad=0)
    ax_in.set_facecolor("white")
    ax_in.patch.set_alpha(0.95)

    denom = max(int(np.isfinite(cat).sum().values), 1)
    counts = np.array([int((cat == k).sum().values) for k in range(4)], dtype=float)
    pct = np.round(counts / denom * 100).astype(int)
    y = np.arange(4)[::-1]
    bar_width = 0.08
    x0 = 0.35
    for i, color in enumerate(colors):
        ax_in.barh(y[i], bar_width, left=x0, height=0.7, color=color, edgecolor="none", zorder=2)
        ax_in.text(x0 - 0.03, y[i], f"{pct[i]}%", va="center", ha="right", fontsize=10, color="0.15")
        ax_in.text(x0 + bar_width + 0.03, y[i], labels[i], va="center", ha="left", fontsize=10, color="0.15")
    ax_in.set_xlim(0, 1)
    ax_in.set_ylim(-0.5, 3.5)
    ax_in.axis("off")
...
def add_colorbar(ax, *, cmap, norm, ticks: Sequence[float], ticklabels: Sequence[str], orientation="vertical", label: str | None = None):
    """Add compact colorbar to an explicit axis."""
    ...
    if label:
        colorbar.set_label(label, rotation=270 if orientation == "vertical" else 0, labelpad=15)
    colorbar.ax.tick_params(direction="in", length=3, width=1, color="white")
    colorbar.outline.set_edgecolor("white")
    return colorbar


def add_colorbar_near_axis(ref_ax, fig, *, cmap, norm, ticks, ticklabels, orientation="vertical", label=""):
    """Add a colorbar positioned relative to another axis."""
    if orientation == "horizontal":
        cax = fig.add_axes([ref_ax.get_position().x0 + 0.15, ref_ax.get_position().y0 + 0.015, ref_ax.get_position().width / 3, 0.008])
    else:
        cax = fig.add_axes([ref_ax.get_position().x0 + 0.03, ref_ax.get_position().y0 + 0.01, 0.01, 0.07])
    return add_colorbar(cax, cmap=cmap, norm=norm, ticks=ticks, ticklabels=ticklabels, orientation=orientation, label=label)
...
def build_figure(cfg: FigureConfig):
    ...
    gs = gridspec.GridSpec(3, 2, figure=fig, height_ratios=(4, 4, 4), width_ratios=(5, 5), wspace=0.10, hspace=0.15)

    cmap_trend, norm_trend = get_truncated_cs("RdBu_r", skip_middle=0.15, vmin=cfg.trend_vmin, vmax=cfg.trend_vmax)
    ...
    plot_climate_zone_decomposition(ax_bar, region_stats, colors=cfg.colors, ylim=(-0.07, 0.07), show_legend=True)
    ...
    plot_trend_map(ax_total, data["ds_total"], title=titles["total"], cmap=cmap_trend, norm=norm_trend, step=3)
    plot_dominance_map(ax_dom, data["ds_lai"], data["ds_sens"])
    plot_trend_map(ax_lai, data["ds_lai"], title=titles["lai"], cmap=cmap_trend, norm=norm_trend, step=3)
    plot_trend_map(ax_sens, data["ds_sens"], title=titles["sens"], cmap=cmap_trend, norm=norm_trend, step=3)
    ...
    for ax in (ax_total, ax_lai, ax_sens):
        add_colorbar_near_axis(ax, fig, cmap=cmap_trend, norm=norm_trend, ticks=ticks, ticklabels=ticklabels, orientation="vertical")

    ax_ts.set_position([ax_total.get_position().x0 + 0.05, ax_ts.get_position().y0 + 0.02, ax_total.get_position().width - 0.05, ax_ts.get_position().height - 0.05])
    ax_bar.set_position([ax_bar.get_position().x0 + 0.05, ax_bar.get_position().y0 + 0.02, ax_bar.get_position().width - 0.05, ax_bar.get_position().height - 0.05])
    ...
    for ax, label, x in [
        ...
        ax.text(x, 1.1, label, transform=ax.transAxes, fontsize=16, fontweight="bold", va="top", ha="right")
