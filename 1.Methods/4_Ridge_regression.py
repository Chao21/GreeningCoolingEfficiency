# Chao Zhang
# Date: 2026
# Purpose: Attribution analysis of LST sensitivity to LAI (Total, Radiative, Non-radiative)
# This script performs pixel-wise temporal attribution using regularized regression.
# It outputs standardized betas, p-values, R2, VIF, physical trend contributions,
# predictor trends, standardized predictor trends, target trends, and target standard deviation.
#
# Major update:
#   - Add a do_detrend switch throughout the attribution workflow.
#   - Default is do_detrend=False, so long-term effects such as CO2 increase are retained.
#   - If do_detrend=True, regression uses detrended standardized time series and mainly reflects
#     interannual co-variability rather than long-term change.
...
# 0. GLOBAL SETTINGS
# -----------------------------------------------------------------------------
# Whether to detrend time series before regression.
# Default: False, because detrending would remove much of the long-term CO2 signal.
DO_DETREND = False
# ---------------------------------------------------------------------
# Moving-window size for pooled attribution
# 1 -> current pixel-wise setting
# 3 -> centered 3x3 moving window
# 5 -> centered 5x5 moving window
# ---------------------------------------------------------------------
WINDOW_SIZE = 1
# Snow cover fraction threshold to separate snow-affected vs snow-free.

Threshold_snowc = 1.0  # percent
# ---------------------------------------------------------------------
# Regression method: "ridge", "ols", or "pls"
# ridge -> Ridge regression with penalty alpha
# ols -> Ordinary least squares multiple linear regression
# pls -> Partial least squares regression using sklearn.cross_decomposition.PLSRegression
# ---------------------------------------------------------------------
REGRESS_METHOD = "ridge" #["ridge", "ols", "pls"]
...
def safe_linregress_slope(y):
    """
    Return linear slope against integer time index, per time step.

    Notes
    -----
    This helper is kept for general use. In the contribution function below,
    a dedicated per-decade slope function is used instead.
    """
...
def run_attribution_workflow(
    ...
    res["pval"].attrs = {
        ...
        "pls_n_permutations": int(pls_n_permutations) if regression_method == "pls" else -1,
    }
    ...
    res["vif"].attrs = {
        ...
        "units": "unitless",
        "note": "Predictor collinearity diagnostic calculated from standardized active predictors",
    }
    ...
    res["predictor_std_trend"].attrs = {"long_name": "Standardized predictor trend per decade"}
    ...
    res.attrs = {
        ...
        "alpha": float(alpha),
        "pls_n_components": int(pls_n_components) if regression_method == "pls" else -1,
        "pls_n_permutations": int(pls_n_permutations) if regression_method == "pls" else -1,
        "pls_random_seed": int(pls_random_seed) if regression_method == "pls" else -1,
...
def run_snow_split_attribution(
    ...
    res_all.attrs["pls_n_components"] = int(pls_n_components) if regression_method == "pls" else -1
    res_all.attrs["pls_n_permutations"] = int(pls_n_permutations) if regression_method == "pls" else -1
    res_all.attrs["pls_random_seed"] = int(pls_random_seed) if regression_method == "pls" else -1
...
# -----------------------------------------------------------------------------
def main():
    os.chdir('/home/energy/chaoz/project/05Veg_LST/data')
    log("Reading datasets...")

    ds_mean_lst = xr.open_dataset(
        'processed/Sensitivity_20260208/GLASS/MOD11C3/Sensitivity_Annual_LSTdailymean_LAI_GLASS_1d.nc'
    )
    ...
    ds_mean_era = xr.open_dataset('ERA5_Land/ERA5_Land_Annual_1d_2000_2024.nc')
    ...
    # Detrending control.
    # Default is False to retain long-term CO2 and climate trends.
    DO_DETREND_RUN = DO_DETREND
    

    # Output directory. Add a suffix to make detrended/non-detrended results explicit.
    outdir = "processed/Attribution_20260827"
    mode_tag = "NoDetrend" if not DO_DETREND_RUN else "Detrend"
    subdir = os.path.join(outdir, f"{REGRESS_METHOD}Regression_{mode_tag}")
    ...
    ds_mean["tp_mean"] = ds_mean_era["tp"]
    ...
    ds_mean["wind_mean"] = ds_mean_era["wind"]
    ...
    is_snow = snowc_clim > Threshold_snowc
    ...
    preds_snow = ["co2_mean","t2m_mean", "swvl1_mean", "ssrd_mean", "vpd_mean", "snowc_mean"]
    preds_nosnow = ["co2_mean","t2m_mean", "swvl1_mean", "ssrd_mean", "vpd_mean"]
    ...
    for target_var in response_vars:
        results_all[target_var] = run_snow_split_attribution(
            ...
            is_snow=is_snow,
            alpha=0.1,                 # Ridge
            regression_method=REGRESS_METHOD,
            time_dim="time",
            lat_dim="lat",
            lon_dim="lon",
            window_size=WINDOW_SIZE,
            std_threshold=1e-6,
            min_valid_n=15,
            do_detrend=DO_DETREND_RUN,
        )
        results_all[target_var].attrs["snow_threshold_percent"] = Threshold_snowc
        results_all[target_var].attrs["detrend"] = int(DO_DETREND_RUN)
    ...
    outfiles = {
        "lst_mean": os.path.join(subdir, f"Attribution_LSTTotal_Annual_winsize{WINDOW_SIZE}.nc"),
        "lst_nonrad_mean": os.path.join(subdir, f"Attribution_LSTNonRadiative_Annual_winsize{WINDOW_SIZE}.nc"),
        "lst_rad_mean": os.path.join(subdir, f"Attribution_LSTRadiative_Annual_winsize{WINDOW_SIZE}.nc"),
    }
