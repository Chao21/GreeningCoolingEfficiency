...
Summary
-------
Create Figure 4: environmental attribution and SEM pathways.
...
Purpose
-------
Compare standardized ridge-regression effects and physical trend contributions
for total, radiative, and nonradiative LST sensitivities, and visualize the
corresponding PLS-SEM pathways.

Notes
-----
Results are displayed separately for snow-affected and snow-free pixels and can
optionally be restricted to dry or humid regions. Set ``VEG_LST_DATA_DIR`` and
...
class FigureConfig:
    ...
    output_root: Path = DATA_ROOT.parent / "figure"
    panel_mode: str = "snow"  # "snow" (global), "dry", or "humid"
    window_size: int = 1
    attribution_dir: Path = (
        DATA_ROOT / "processed/Attribution_20260827/ridgeRegression_NoDetrend"
    )
    sem_dir: Path = (
        DATA_ROOT
        / "processed/Attribution_20260827/PLSEM/regional_summary_tables_20260828"
    )
...
def output_path(cfg: FigureConfig) -> Path:
    if cfg.panel_mode == "snow":
        return cfg.output_root / "Fig.04_Attribution_Global.svg"
    if cfg.panel_mode == "dry":
        return cfg.output_root / "Fig.04_Attribution_Drylands.svg"
    if cfg.panel_mode == "humid":
        return cfg.output_root / "Fig.04_Attribution_Humidlands.svg"
    raise ValueError("panel_mode must be one of {'snow', 'dry', 'humid'}")


def get_ylim(cfg: FigureConfig) -> tuple[tuple[float, float], tuple[float, float]]:
    """Return y-axis limits for the two panels."""
    if cfg.panel_mode == "snow":
        return (-0.7, 0.7), (-0.25, 0.25)

    return (-0.8, 0.8), (-0.38, 0.38)



def sem_path(cfg: FigureConfig) -> Path:
    if cfg.panel_mode == "snow":
        filename = "semcoeff_global_by_snow_winsize1.csv"
    elif cfg.panel_mode in {"dry", "humid"}:
        filename = "semcoeff_dryhumid_by_snow_winsize1.csv"
    else:
        raise ValueError("panel_mode must be one of {'snow', 'dry', 'humid'}")
    return cfg.sem_dir / filename


def sem_region_name(cfg: FigureConfig) -> str:
    return {"snow": "Global", "dry": "Dry", "humid": "Humid"}[cfg.panel_mode]


def panel_titles(cfg: FigureConfig) -> tuple[str, str, str]:
    if cfg.panel_mode == "snow":
...
def load_attribution_datasets(cfg: FigureConfig) -> dict[str, xr.Dataset]:
    ...
    return {
        "LSTtotal": xr.load_dataset(
            root / f"Attribution_LSTTotal_Annual_winsize{w}.nc"
        ),
        "LSTrad": xr.load_dataset(
            root / f"Attribution_LSTRadiative_Annual_winsize{w}.nc"
        ),
        "LSTnonrad": xr.load_dataset(
            root / f"Attribution_LSTNonRadiative_Annual_winsize{w}.nc"
        ),
    }
...
def field_to_long_df(
    ...
    *,
    ...
    region_mask = region_mask.sel(
        {lat_dim: da_field[lat_dim], lon_dim: da_field[lon_dim]},
        method="nearest",
    )
...
def build_long_df_for_dataset(
    ...
    *,
    ...
    pval_mask = (
        xr.ones_like(ds[variable], dtype=bool)
        if pval_threshold is None
        else ds["pval"] < pval_threshold
    )
...
def plot_effect_panel(
    ...
    *,
    ...
    for i, response in enumerate(RESPONSE_ORDER):
        ...
        print(f'value_name:{value_name}, {response} ', stats)
...
def draw_arrow(
    ...
    *,
    ...
    ax.text(
        xm, ym, f"{beta:.2f}", fontsize=12, ha="center", va="center",
        rotation=label_rotation, zorder=31,
    )
...
def draw_routed_arrow(
    ...
    *,
    ...
    for p0, p1 in zip(points[:-2], points[1:-1]):
        ax.plot(
            [p0[0], p1[0]], [p0[1], p1[1]], color=color,
            linewidth=linewidth, linestyle=linestyle, zorder=8,
        )
    ...
    ax.text(
        label_xy[0], label_xy[1], f"{beta:.2f}", fontsize=12,
...
def plot_sem_panel(ax, coef: Mapping[str, object], title: str = "", *, show_snow: bool = True):
    ...
    routed_paths = [
        (
            [(pos["SM"][0], pos["SM"][1] + 0.02), (0.00, 0.91),
             (0.00, 0.20), pos["SEN_NR"]],
            "SM->SEN_NR", (0.24, 0.94), 0,
        ),
        (
            [(pos["VPD"][0] - 0.08, pos["VPD"][1]), (0.02, 0.74),
             (0.02, 0.20), pos["SEN_NR"]],
            "VPD->SEN_NR", (0.04, 0.61), -90,
        ),
        (
            [(pos["SM"][0], pos["SM"][1] + 0.02), (1.02, 0.91),
             (1.02, 0.20), pos["SEN_R"]],
            "SM->SEN_R", (0.74, 0.94), 0,
        ),
        (
            [(pos["SEN_NR"][0], pos["SEN_NR"][1] - 0.06),
             (0.14, -0.01), pos["SEN_TOT"]],
            "SEN_NR->SEN_TOT", (0.28, 0.03), 0,
        ),
        (
            [(pos["SEN_R"][0], pos["SEN_R"][1] - 0.06),
             (0.86, -0.01), pos["SEN_TOT"]],
            "SEN_R->SEN_TOT", (0.70, 0.03), 0,
        ),
    ]
...
def build_figure(cfg: FigureConfig) -> plt.Figure:
    ...
    focus_region, left_label, right_label = panel_titles(cfg)
    ds_dict = load_attribution_datasets(cfg)

    aridity = xr.load_dataarray(cfg.base_dir / "AI_180x360.nc")
    ...
    sem_df = pd.read_csv(sem_path(cfg))
    region = sem_region_name(cfg)
    ...
    plot_effect_panel(
        ...
        ylim=ylim_cd,
        ylabel=(
            r"Contribution to temporal trend"
            "\n"
...
def main() -> None:
    cfg = FigureConfig(panel_mode="snow")
    build_figure(cfg)
